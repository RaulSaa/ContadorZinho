...
const FinanceTracker = ({ auth, db, familyId }) => {
    ...
    // O restante das funções (importação de CSV, etc.) permanece funcionalmente o mesmo, apenas com os caminhos atualizados.
    // Para manter o código breve, eles não foram repetidos aqui.
    ...
    return (
        <div className="min-h-screen bg-gray-100 flex flex-col justify-center items-center p-4">
            <div className="max-w-md w-full bg-white p-8 rounded-xl shadow-lg">
                ...
                <p className="text-center text-gray-600 mb-6">Crie um novo espaço para compartilhar ou entre em um existente com um código de convite.</p>
...
// --- COMPONENTE PRINCIPAL QUE GERENCIA A VISUALIZAÇÃO ---
            ...
            const unsubscribe = onAuthStateChanged(firebaseAuth, async (currentUser) => {
                if (currentUser) {
                    ...
                    if (userDocSnap.exists() && userDocSnap.data().familyId) {
                        ...
                        // Novo usuário ou usuário sem família
