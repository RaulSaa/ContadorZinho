...
const FinanceTracker = ({ auth, db, userId }) => {
    ...
    useEffect(() => {
        if (db && userId) {
            ...
            const transactionsCollection = collection(db, `${path}/transactions`);
    ...
    const addTransaction = (e, responsible) => {
        ...
        addDoc(collection(db, `users/${userId}/transactions`), data)
            .then(() => {
    ...
    // ... (restante da lógica do tracker, como delete, classify, etc.)
                 ...
                 <section className="bg-white p-6 rounded-xl shadow-lg">
                  ...
                  <form className="space-y-4">
                    <div className="flex flex-col sm:flex-row gap-4">
                      <input id="description" type="text" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Descrição" className="flex-1 mt-1 block w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-md shadow-sm"/>
                      <input id="amount" type="number" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="Valor" className="flex-1 mt-1 block w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-md shadow-sm"/>
                    </div>
                    <div className="flex flex-col sm:flex-row gap-4">
                      <select id="type" value={type} onChange={(e) => setType(e.target.value)} className="flex-1 mt-1 block w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-md shadow-sm"><option value="receita">Receita</option><option value="despesa">Despesa</option></select>
                      <input id="date" type="date" value={manualDate} onChange={(e) => setManualDate(e.target.value)} className="flex-1 mt-1 block w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-md shadow-sm"/>
                    </div>
            ...
            <div className="max-w-md w-full bg-white p-8 rounded-xl shadow-lg">
                ...
                <form onSubmit={handleAuthAction} className="space-y-6">
                    <div>
                        ...
                        <input id="email" name="email" type="email" required value={email} onChange={e => setEmail(e.target.value)} className="mt-1 block w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-md shadow-sm"/>
                    </div>
                    <div>
                        <label htmlFor="password" className="block text-sm font-medium text-gray-700">Senha</label>
                        <input id="password" name="password" type="password" required value={password} onChange={e => setPassword(e.target.value)} className="mt-1 block w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-md shadow-sm"/>
                    </div>
