...
const CalendarView = ({ db, userId }) => {
    ...
    const handleAddEvent = async () => {
        ...
        await addDoc(collection(db, `users/${userId}/calendarEvents`), {
            title: eventTitle, start: startDateTime, end: endDateTime, frequency, reminder, createdAt: serverTimestamp()
        });
        setIsEventModalOpen(false);
        setEventTitle(''); setStartDate(''); setStartTime('09:00'); setEndDate(''); setEndTime('10:00'); setFrequency('none'); setReminder('none');
    };
        ...
        <div className="p-4 md:p-8">
            ...
            <header className="p-6 bg-white rounded-xl shadow-lg text-center mb-8">
                 ...
                 <div className="mt-4">
                    ...
                    <button onClick={() => setViewMode('week')} className={`py-1 px-3 text-sm rounded-r-lg ${viewMode === 'week' ? 'bg-gray-200' : 'bg-gray-200'}`}>Semana</button>
                 </div>
            </header>
